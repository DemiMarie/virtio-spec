...
Driver auxiliary notifications are not supported when IntX is used.
...
The \field{cap.offset} and \field{dev_aux_notification_off_multiplier}
are taken from the device auxiliary notification capability
structure above, and the \field{dev_aux_notification_idx} is the
device auxiliary notification index. There is no restriction on
the mapping between device auxiliary notifications and
dev_aux_notification_idx. The number of device auxiliary
notifications and their purpose is device-specific.
...
\field{dev_aux_notification_off_multiplier} MUST be either zero,
or a power of 2 that is a multiple of the data width.
The value \field{cap.length} presented by the device MUST be at
least the data width and MUST be large enough to support device
auxiliary notification offsets for all supported device auxiliary
notifications, regardless of what the driver does.

The value \field{cap.length} presented by the device MUST satisfy:

\begin{lstlisting}
cap.length >= max_dev_aux_notification_idx * dev_aux_notification_off_multiplier + data_width
\end{lstlisting}

where \field{max_dev_aux_notification_idx} is the maximum device
auxiliary notification index and is dependent on the device.
...
configured for notification interrupts. If MSI-X is not
available, auxiliary notifications are not supported.
...
change the MSI-X vector for that notification. Alternatively, the driver can
use a single 32-bit write to both fields, which has the same effect.

The mapping between notifications and notification indices is device-specific.
The total number of notifications is also device-specific.

\begin{note}
While it would be possible to support a single auxiliary
notification without MSI-X, this would not be very useful.
\end{note}.
...
Devices MUST ignore writes to \field{driver_aux_notification_msix_vector} if
the value written to \field{driver_aux_notification_select} is not a valid
notification index. This allows drivers to write the value they expect first,
and check if the write succeeded afterwards.

Devices MUST return VIRTIO_MSI_NO_VECTOR for reads from
...
Devices MUST NOT write to the notification capability after setting DRIVER_OK.
...
The device MUST have all queue, configuration change, and
device-specific events are unmapped upon reset.
...
\field{config_msix_vector}/\field{queue_msix_vector}\field{driver_aux_notification_msix_vector} field
is read.
